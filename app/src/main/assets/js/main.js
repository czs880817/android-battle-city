    ...
    switch (gameState) {
        case GAME_STATE_MENU:
            if (event == keyboard.TOUCH_START) {
                ...
                var n = 0;
                if (event == keyboard.TOUCH_DOWN) {
            		n = 1;
            	} else if (event == keyboard.TOUCH_UP) {
            		n = -1;
            	}
            	menu.next(n);
            }
            break;
        case GAME_STATE_START:
            if (!keys.contain(event)) {
        		keys.push(event);
        	}

        	if (event == keyboard.TOUCH_START && player1.lives > 0){
...
function keyEvent(){
	if(keys.contain(keyboard.W)||keys.contain(keyboard.TOUCH_UP)){
		player1.dir = UP;
		player1.hit = false;
		player1.move();
	}else if(keys.contain(keyboard.S)||keys.contain(keyboard.TOUCH_DOWN)){
		player1.dir = DOWN;
		player1.hit = false;
		player1.move();
	}else if(keys.contain(keyboard.A)||keys.contain(keyboard.TOUCH_LEFT)){
		player1.dir = LEFT;
		player1.hit = false;
		player1.move();
	}else if(keys.contain(keyboard.D)||keys.contain(keyboard.TOUCH_RIGHT)){
		player1.dir = RIGHT;
		player1.hit = false;
		player1.move();
	}
